...
CMD_PETBAR_BUTTONS        = "  |cffffff00/uui petbar size <12-64> / spacing <0-32> / reset|r - размер и отступ стандартных кнопок питомца",
PETBAR_BUTTONS_APPLIED    = "Кнопки питомца: размер %d, отступ %d.",
PETBAR_BUTTONS_NATIVE     = "Кнопки питомца: значения клиента (размер %d, отступ %d).",
PETBAR_BUTTONS_RESTORED   = "Кнопки питомца: восстановлены размер и отступ клиента.",
PETBAR_BUTTONS_RANGE      = "Размер кнопок питомца: %d-%d, отступ: %d-%d.",
...
SETTINGS_AUTO_ATTACK_HINT = "Щелчок мыши или Tab начинает атаку. Поместите «Автоматическую стрельбу» (охотник) или «Выстрел» на панель команд для переключения лука/ружья и ближнего боя по расстоянию. Учитывает ручную остановку. Останавливает атаку и не начинает новую, пока разбойник или друид скрыт.",
...
SETTINGS_TOOLTIP_CURSOR  = "Подсказки мира следуют за курсором",
SETTINGS_TOOLTIP_CURSOR_HINT = "Подсказки мира остаются рядом с курсором и в пределах экрана. Отключите, чтобы использовать сохранённую позицию подсказки.",
SETTINGS_CHAT_SHADOW      = "Убрать тень текста чата",
SETTINGS_CHAT_SHADOW_RELOAD = "Введите /reload, чтобы восстановить тень текста чата.",
SETTINGS_CHAT_SHADOW_HINT = "Сообщения чата рисуются плоско, без тени за буквами. Подписи вкладок чата не затрагиваются.",
...
ABC_PET_BAR              = "Панель питомца",
ABC_PET_RESET            = "Вернуть размер клиента",
ABC_PET_HINT             = "Эти настройки размещают собственные кнопки питомца клиента, поэтому здесь нет включения и количества кнопок. Движение любого ползунка передаёт ряд аддону; «Вернуть размер клиента» возвращает его игре. То же самое, что /uui petbar size, spacing и reset.",
ABC_PET_UNAVAILABLE      = "Родная панель питомца не активна в этой сессии, поэтому её кнопки нельзя разместить.",
...
-- Подразделы группы "Рамки существ" в окне настроек.
UF_TAB_GENERAL            = "Общие настройки",
UF_TAB_PARTY              = "Рамки группы",
UF_TAB_AURAS              = "Эффекты",
UF_TAB_COLORS             = "Цвета",

-- Unit frame style (core/unitframestyle.lua). See enUS.lua: the style names
-- stay untranslated, the same as the theme names.
UF_STYLE_HEADER           = "Стиль рамок существ",
UF_STYLE_HINT             = "«Minimal» — стиль рамок, который unrealUI рисует сейчас. «Enhanced» ещё в разработке и пока недоступен.",
UF_STYLE_CHANGED          = "Стиль рамок изменён",
UF_STYLE_RELOAD           = "Введите /reload, чтобы применить стиль рамок «%s».",
...
UF_OFFLINE                = "НЕ В СЕТИ",

UF_EXACT_VITALS_HEADER    = "Здоровье существ",
UF_EXACT_VITALS           = "Показывать точное здоровье и ману",
UF_EXACT_VITALS_HINT      = "Клиент сообщает здоровье существа только в процентах. Когда опция включена, рамка цели показывает реальные значения из встроенной таблицы существ и уточняет текущее по нанесённому урону. Выключите, чтобы видеть проценты.",
...
-- Индикаторы периодического лечения на рамках группы (modules/hots.lua).
HOTS_HEADER               = "Индикаторы периодического лечения",
HOTS_ENABLED              = "Показывать периодическое лечение",
HOTS_CORNER               = "Угол",
HOTS_SIZE                 = "Размер значков",
HOTS_SPACING              = "Отступ между значками",
HOTS_CORNER_TOPLEFT       = "Сверху слева",
HOTS_CORNER_TOPRIGHT      = "Сверху справа",
HOTS_CORNER_BOTTOMLEFT    = "Снизу слева",
HOTS_CORNER_BOTTOMRIGHT   = "Снизу справа",
HEALPREDICT_HEADER        = "Входящее лечение",
HEALPREDICT_ENABLED       = "Показывать входящее лечение на полосах здоровья",
HEALPREDICT_HINT          = "Закрашивает ту часть полосы здоровья, которую заполнит ваше лечение, пока оно ещё читается, чтобы избыточное лечение было видно заранее. Клиент не сообщает о входящем лечении, поэтому показываются только ваши собственные заклинания, а величина лечения становится известна лишь после того, как оно один раз сработало. Лечение других лекарей никогда не отображается.",
...
-- Bags and bank
-- ---------------------------------------------------------------------------
SETTINGS_BAGS_ENABLE      = "Включить",
SETTINGS_BAGS_ENABLE_HINT = "Заменяет окна сумок клиента общей сумкой UnrealUI. Если снять флажок, сумка UnrealUI полностью отключается и возвращаются стандартные сумки клиента, чтобы другой аддон для сумок мог работать без конфликтов. Любое изменение требует перезагрузки.",
SETTINGS_BAGS_CHANGED     = "Интерфейс сумок изменён",
SETTINGS_BAGS_RELOAD_ON   = "Введите /reload, чтобы использовать сумку UnrealUI.",
SETTINGS_BAGS_RELOAD_OFF  = "Введите /reload, чтобы вернуть стандартные сумки клиента.",
SETTINGS_BAGS_CATEGORIES  = "Сумка по категориям",
SETTINGS_BAGS_CATEGORIES_HINT = "Группирует содержимое сумки в подписанные блоки категорий вместо единой сетки. Категории берутся из класса самого предмета, поэтому ничего не нужно сортировать или помечать вручную. Применяется сразу, перезагрузка не требуется.",

ITEM_CATEGORY_FAVORITE    = "Избранное",
ITEM_CATEGORY_GEAR        = "Моё снаряжение",
ITEM_CATEGORY_QUEST       = "Предметы заданий",
ITEM_CATEGORY_CONSUMABLE  = "Расходники",
ITEM_CATEGORY_FOOD        = "Еда и напитки",
ITEM_CATEGORY_POTION      = "Зелья",
ITEM_CATEGORY_BUFF        = "Усиления",
ITEM_CATEGORY_BANDAGE     = "Бинты",
ITEM_CATEGORY_EXPLOSIVE   = "Взрывчатка",
ITEM_CATEGORY_REAGENT     = "Реагенты класса",
ITEM_CATEGORY_TRADEGOODS  = "Хозяйственные товары",
ITEM_CATEGORY_RECIPE      = "Рецепты",
ITEM_CATEGORY_AMMO        = "Боеприпасы",
ITEM_CATEGORY_CONTAINER   = "Контейнеры",
ITEM_CATEGORY_KEY         = "Ключи",
ITEM_CATEGORY_JUNK        = "Хлам",
ITEM_CATEGORY_MISC        = "Разное",
ITEM_CATEGORY_UNKNOWN     = "Без категории",

BAGS_TITLE                = "Сумки",
BAGS_SLOT_COUNT           = "%d/%d",
BAGS_SORT                 = "Сортировка сумок",
BAGS_SORT_HINT            = "Переставляет содержимое сумок по категориям. Специальные сумки (травы, души, чары, инженерное дело, колчаны) не затрагиваются.",
BAGS_SORT_BUSY            = "Сортировка уже идёт.",
BAGS_SORT_CURSOR          = "Сначала положите предмет с курсора.",
BAGS_SORT_DONE            = "Сумки отсортированы.",
BAGS_SORT_NOTHING         = "Сумки уже отсортированы.",
BAGS_SORT_FAILED          = "Сортировка остановлена: предмет не переместился. Ничего не потеряно.",
BAGS_SORT_NOSTAGE         = "Для сортировки нужна одна свободная ячейка в сумке. Освободите ячейку и повторите.",
...
BAGS_FAVORITE_HINT_ADD    = "Alt + ЛКМ: отметить как избранное",
BAGS_FAVORITE_HINT_REMOVE = "Alt + ЛКМ: убрать из избранного",
BAGS_FAVORITE_SELL_CONFIRM = "Продать этот избранный предмет?",
BAGS_FAVORITE_SELL_ACCEPT = "Продать",
BAGS_FAVORITE_BATCH_SELL_ONE   = "%d серый предмет отмечен как избранный.",
BAGS_FAVORITE_BATCH_SELL_FEW   = "%d серых предмета отмечены как избранные.",
BAGS_FAVORITE_BATCH_SELL_MANY  = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_SELL_OTHER = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_ONE   = "%d серый предмет отмечен как избранный.",
BAGS_FAVORITE_BATCH_DELETE_FEW   = "%d серых предмета отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_MANY  = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_OTHER = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DETAIL = "Включить их или пропустить и оставить в сумках?",
BAGS_FAVORITE_BATCH_INCLUDE = "Включить",
BAGS_FAVORITE_BATCH_SKIP  = "Пропустить",
BAGS_FAVORITE_BATCH_NONE_LEFT = "Найдены только избранные серые предметы; ничего не сделано.",
BANK_TITLE                = "Банк",
BANK_SORT                 = "Сортировка банка",
BANK_SORT_HINT            = "Переставляет содержимое банка и банковских сумок по категориям. Специальные сумки не затрагиваются.",
BANK_SORT_BUSY            = "Сортировка уже идёт.",
BANK_SORT_CURSOR          = "Сначала положите предмет с курсора.",
BANK_SORT_DONE            = "Банк отсортирован.",
BANK_SORT_NOTHING         = "Банк уже отсортирован.",
BANK_SORT_FAILED          = "Сортировка банка остановлена: предмет не переместился.",
BANK_SORT_NOSTAGE         = "Для сортировки банка нужна одна свободная ячейка в банковской сумке. Освободите ячейку и повторите.",
...
XPTIP_TITLE               = "Опыт",
XPTIP_XP                  = "Опыт",
XPTIP_REMAINING           = "Осталось",
XPTIP_STATUS              = "Состояние",
XPTIP_RESTING             = "Отдых",
XPTIP_RESTED              = "Отдохнувший",
XPTIP_SESSION             = "За сессию",
XPTIP_PER_HOUR            = "В среднем за час",
XPTIP_TIME_LEFT           = "Осталось времени",
