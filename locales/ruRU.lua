-- unrealUI :: locales/ruRU.lua
--
-- Russian. See locales/enUS.lua for the catalog and the rules every locale
-- file follows (UTF-8 without BOM, placeholders kept in order, colour escapes
-- kept paired, slash commands never translated).
--
-- Russian is the reason core/locale.lua carries U.LN rather than a single
-- "one / other" pair: a counted noun takes three different forms, and the
-- form depends on the last one or two digits, not on whether the count is 1.

UnrealUI.RegisterLocale("ruRU", {

-- Opt-in custom pet bar (native remains the default).
CMD_PETBAR                = "  |cffffff00/uui petbar <native/custom>|r - выбрать панель питомца (нужен /reload)",
PETBAR_MODE_NATIVE        = "Стандартная",
PETBAR_MODE_CUSTOM        = "Пользовательская (Modern)",
PETBAR_MODE_STATUS        = "Панель питомца: %s. После перезагрузки: %s.",
PETBAR_MODE_SELECTED      = "Выбрана панель питомца: %s. Введите /reload для применения.",
PETBAR_CUSTOM_WARNING     = "Экспериментальная панель питомца: клиент блокирует применение заклинаний щелчком. Верните стандартную панель командой /uui petbar native, затем /reload.",
PETBAR_ACTION_UNAVAILABLE = "Это действие питомца недоступно в данном клиенте.",
PETBAR_UNAVAILABLE        = "Выбор панели питомца недоступен в этом сеансе.",
CMD_PETBAR_BUTTONS        = "  |cffffff00/uui petbar size <12-64> / spacing <0-32> / reset|r - размер и отступ стандартных кнопок питомца",
PETBAR_BUTTONS_APPLIED    = "Кнопки питомца: размер %d, отступ %d.",
PETBAR_BUTTONS_NATIVE     = "Кнопки питомца: значения клиента (размер %d, отступ %d).",
PETBAR_BUTTONS_RESTORED   = "Кнопки питомца: восстановлены размер и отступ клиента.",
PETBAR_BUTTONS_RANGE      = "Размер кнопок питомца: %d-%d, отступ: %d-%d.",

-- ---------------------------------------------------------------------------
-- Shared controls
-- ---------------------------------------------------------------------------
COMMON_OK                 = "Хорошо",
COMMON_OK_SHORT           = "ОК",
COMMON_CANCEL             = "Отмена",
COMMON_ACCEPT             = "Принять",
COMMON_CLOSE              = "Закрыть",
COMMON_SAVE               = "Сохранить",
COMMON_DELETE             = "Удалить",
COMMON_ENABLE             = "Включить",
COMMON_ARE_YOU_SURE       = "Вы уверены?",
COMMON_CANNOT_BE_UNDONE   = "Это действие нельзя отменить.",
COMMON_SELECT_COLOUR      = "Выбор цвета",
COMMON_COST               = "Цена:",
COMMON_SELL               = "Продажа:",
LOADING_TITLE             = "Подготовка UnrealUI",
LOADING_CLIENT_LIMITATION = "Загрузка нужна из-за ограничений  клиента. Чтобы избежать зависаний во время игры.",
LOADING_FPS_DEPENDENT     = "Длительность зависит от FPS. Будет убрано в будущем обновлении клиента.",

-- ---------------------------------------------------------------------------
-- Edit mode
-- ---------------------------------------------------------------------------
MOVER_TITLE               = "Редактор интерфейса",
MOVER_HINT_MAGNET         = "|cfff5ae0aРядом с другим элементом|r: примагничивание",
MOVER_HINT_ARROWS         = "|cfff5ae0aЩелчок по рамке|r: стрелки смещают на 1 пиксель",
MOVER_GRID                = "Показывать сетку выравнивания",
MOVER_MAGNET              = "Примагничивать элементы (без наложения)",
MOVER_GROUP               = "Перемещать соприкасающиеся якоря вместе",
MOVER_GRID_SIZE           = "Размер сетки",
MOVER_ADVANCED_ANCHORS    = "Дополнительные привязки",
MOVER_SAVE_EXIT           = "Сохранить и выйти",
MOVER_RESET               = "Сбросить",
MOVER_DRAG_FIRST          = "Перетащите «%s» один раз, прежде чем смещать стрелками.",
MOVER_ENTERED             = "Режим редактирования. Перетаскивайте, примагничивайте или смещайте стрелками. |cffffff00Сохранить и выйти|r по завершении.",
MOVER_SAVED               = "Расположение сохранено. Режим редактирования закрыт.",
MOVER_RESET_ONE           = "Сброшена %d позиция рамки к значениям по умолчанию.",
MOVER_RESET_FEW           = "Сброшены %d позиции рамок к значениям по умолчанию.",
MOVER_RESET_MANY          = "Сброшено %d позиций рамок к значениям по умолчанию.",
MOVER_RESET_OTHER         = "Сброшено %d позиций рамок к значениям по умолчанию.",

MOVER_LABEL_PLAYER        = "Игрок",
MOVER_LABEL_TARGET        = "Цель",
MOVER_LABEL_TARGET_TARGET = "Цель цели",
MOVER_LABEL_PET           = "Питомец",
MOVER_LABEL_PET_UNIT_FRAME = "Рамка питомца",
MOVER_LABEL_PARTY         = "Группа",
MOVER_LABEL_PARTY_N       = "Группа %d",
MOVER_LABEL_DRUID_MANA    = "Мана друида",
MOVER_LABEL_CASTBAR       = "Полоса заклинаний",
MOVER_LABEL_TARGET_CASTBAR = "Полоса заклинаний цели",
MOVER_LABEL_PET_CASTBAR   = "Полоса заклинаний питомца",
CASTBAR_FAILED            = "Неудача",
CASTBAR_INTERRUPTED       = "Прервано",
CHARACTER_CLASS_LEVEL     = "%s |cffffffff%d-го уровня|r",
MOVER_LABEL_XP_BAR        = "Полоса опыта",
MOVER_LABEL_REP_BAR       = "Полоса репутации",
MOVER_LABEL_BREATH_BAR    = "Полоса дыхания",
MOVER_LABEL_SWING_BAR     = "Автоатака: полоса атаки",
MOVER_LABEL_MICRO_BAR     = "Микропанель",
MOVER_LABEL_BAG_BAR       = "Панель сумок",
MOVER_LABEL_BUFFS         = "Усиления и ослабления",
MOVER_LABEL_MINIMAP       = "Миникарта",
MOVER_LABEL_DURABILITY    = "Прочность брони",
MOVER_LABEL_TOOLTIP       = "Подсказка",
MOVER_LABEL_PET_BAR       = "Панель питомца",
MOVER_LABEL_STANCE_BAR    = "Панель стоек",
MOVER_LABEL_STATUS        = "Панель состояния",
MOVER_LABEL_ONLINE_COUNT  = "Счётчик игроков онлайн",
MOVER_LABEL_QUEST_TRACKER = "Отслеживание заданий",
MOVER_LABEL_CRAFT_TRACKER = "Отслеживание ремесла",
MOVER_LABEL_ACTION_BAR    = "Панель %d",
MOVER_LABEL_MOVER_TEST    = "Проверка перемещения",

-- ---------------------------------------------------------------------------
-- Grouped game settings window (modules/gamesettings.lua)
-- ---------------------------------------------------------------------------
GAMESETTINGS_TITLE        = "Настройки игры",
GAMESETTINGS_VIDEO        = "Видео",
GAMESETTINGS_SOUND        = "Звук",
GAMESETTINGS_INTERFACE    = "Интерфейс",
GAMESETTINGS_KEYBINDINGS  = "Горячие клавиши",
GAMESETTINGS_UNREALUI     = "Unreal UI",
GAMESETTINGS_UNREALQUEST  = "Unreal Quest",
GAMESETTINGS_UNAVAILABLE  = "Эта панель недоступна на данном клиенте.",
GAMESETTINGS_SEARCH       = "Поиск",
GAMESETTINGS_NO_MATCHES   = "Подходящие параметры не найдены.",

-- ---------------------------------------------------------------------------
-- Settings window
-- ---------------------------------------------------------------------------
SETTINGS_MOVE_UI          = "Переместить интерфейс",
SETTINGS_LANGUAGE_CHANGED = "Язык изменён",
SETTINGS_LANGUAGE_RELOAD  = "Введите /reload, чтобы отобразить UnrealUI на языке: %s.",

SETTINGS_PAGE_GENERAL     = "Общие",
SETTINGS_PAGE_PROFILES    = "Профили",
SETTINGS_PAGE_BAGS        = "Сумки",
SETTINGS_PAGE_ROGUE       = "Разбойник",

SETTINGS_THEME_STYLE      = "Стиль оформления",
SETTINGS_THEME_CHANGED    = "Оформление изменено",
SETTINGS_THEME_RELOAD     = "Введите /reload, чтобы применить оформление «%s».",
SETTINGS_THEME_HINT       = "«Classic WoW» после перезагрузки восстанавливает исходный интерфейс клиента, сохраняя пункты UnrealUI и «Быстрые привязки» в игровом меню. «Modern WoW» ещё в разработке.",
SETTINGS_THEME_WIP        = " (в разработке)",
SETTINGS_THEMES_AVAILABLE = "доступные оформления:",

CLASSIC_MODULES_PAGE         = "Модули Modern WoW",
CLASSIC_MODULES_INTRO        = "Импортируйте любой модуль темы Modern WoW в тему Classic WoW.",
CLASSIC_MODULES_HINT         = "Перезагрузите интерфейс, чтобы применить изменения.",
CLASSIC_MODULES_RELOAD       = "Введите /reload, чтобы применить изменения модулей Modern WoW.",
CLASSIC_MODULE_UNIT_FRAMES   = "Рамки персонажей",
CLASSIC_MODULE_PARTY_FRAMES  = "Рамки группы",
CLASSIC_MODULE_ACTION_BARS   = "Панели команд",
CLASSIC_MODULE_MINIMAP       = "Миникарта",
CLASSIC_MODULE_CHARACTER     = "Окно персонажа",
CLASSIC_MODULE_SPELLBOOK     = "Книга заклинаний",
CLASSIC_MODULE_TALENTS       = "Окно талантов",
CLASSIC_MODULE_CRAFTING      = "Окно профессий",
CLASSIC_MODULE_QUEST_LOG     = "Журнал заданий",
CLASSIC_MODULE_BAGS          = "Сумки и банк",
CLASSIC_MODULE_LOOT          = "Окно добычи",

SETTINGS_QUICKBIND        = "Быстрые привязки",
SETTINGS_QUICKBIND_HINT   = "Наведите курсор на ячейку панели команд или панели стоек и нажмите клавишу, чтобы назначить её. Escape над ячейкой очищает привязку.",
SETTINGS_AUTO_ATTACK      = "Начинать автоатаку при выборе цели",
SETTINGS_AUTO_ATTACK_HINT = "Щелчок мыши или Tab начинает атаку. Переключает лук/ружьё и ближний бой по расстоянию; охотнику без «Автоматической стрельбы» на панели она ставится в пустую ячейку. Учитывает ручную остановку. Останавливает атаку и не начинает новую, пока разбойник или друид скрыт.",
SETTINGS_SWING_BAR        = "Показывать полосу автоатаки",
SETTINGS_SWING_BAR_HINT   = "Показывает таймеры основной, дополнительной и луковой или дальней атаки, пока цель находится в радиусе действия. Перемещается через режим интерфейса.",
SETTINGS_MICROBAR         = "Включить микропанель",
SETTINGS_MICROBAR_HINT    = "Собирает стандартные кнопки персонажа, книги заклинаний, талантов, журнала заданий, социального окна, карты, меню и помощи в один перемещаемый ряд. Отключение возвращает их на штатные места.",
SETTINGS_REPUTATION_BAR   = "Показывать полосу репутации",
SETTINGS_MINIMAP_BUTTON   = "Показывать кнопку настроек у миникарты",
SETTINGS_ZONE_LEVELS      = "Показывать диапазоны уровней зон на карте мира",
SETTINGS_ZONE_LEVELS_HINT = "При наведении на зону на карте континента рядом с названием показывается её диапазон уровней: зелёный — ниже вашего уровня, оранжевый — на вашем уровне, красный — выше вашего уровня.",
SETTINGS_TOOLTIP_CURSOR  = "Подсказки мира следуют за курсором",
SETTINGS_TOOLTIP_CURSOR_HINT = "Подсказки мира остаются рядом с курсором и в пределах экрана. Отключите, чтобы использовать сохранённую позицию подсказки.",
SETTINGS_TOOLTIP_FADE_HOLD = "Задержка скрытия подсказки (секунды)",
SETTINGS_CHAT_SHADOW      = "Убрать тень текста чата",
SETTINGS_CHAT_SHADOW_RELOAD = "Введите /reload, чтобы восстановить тень текста чата.",
SETTINGS_CHAT_SHADOW_HINT = "Сообщения чата рисуются плоско, без тени за буквами. Подписи вкладок чата не затрагиваются.",

SWING_BAR_WIDTH           = "Ширина полосы",
SWING_BAR_HEIGHT          = "Высота полосы",
SWING_BAR_MAIN            = "Осн.",
SWING_BAR_OFF             = "Доп.",
SWING_BAR_RANGED          = "Дал.",

ROGUE_SETTINGS_HEADER         = "Быстрое нанесение ядов",
ROGUE_POISON_SHIFT_CLICK      = "Включить нанесение ядов по Shift",
ROGUE_POISON_SHIFT_CLICK_HINT = "В сумках нажмите Shift + левую кнопку мыши на яде для правой руки или Shift + правую кнопку для левой руки. При открытой строке чата ссылки на предметы работают как обычно.",

PROFILE_SELECT            = "Выбрать профиль",
PROFILE_SELECT_HINT       = "Выберите профиль, созданный любым персонажем этой учётной записи.",
PROFILE_NONE_OTHER        = "Других профилей нет",
PROFILE_CREATE_COPY       = "Создать копию профиля",
PROFILE_CREATE_HINT       = "Копирует текущие настройки под сгенерированным именем. Для своего имени используйте /uui profile create <имя>.",
PROFILE_COPY_FROM         = "Копировать из",
PROFILE_COPY_SETTINGS     = "Копировать настройки",
PROFILE_COPY_HINT         = "Копирует другой профиль в текущий активный профиль.",
PROFILE_COPIED            = "профиль |cffffff00%s|r скопирован в |cffffff00%s|r",
PROFILE_DELETE_SECTION    = "Удалить профиль",
PROFILE_DELETE            = "Удалить профиль",
PROFILE_DELETE_HINT       = "Удаляет профиль, не назначенный ни одному персонажу.",
PROFILE_DELETED           = "удалён профиль |cffffff00%s|r",
PROFILE_CONFIRM_DELETE    = "Подтвердить удаление",
PROFILE_RESET_SECTION     = "Сбросить текущий профиль",
PROFILE_RESET             = "Сбросить профиль",
PROFILE_CONFIRM_RESET     = "Подтвердить сброс",
PROFILE_RESET_HINT        = "Восстанавливает значения по умолчанию. Персонажи с тем же профилем используют общие настройки.",
PROFILE_CURRENT           = "Текущий профиль: |cfff5ae0a%s|r",
PROFILE_CLICK_CONFIRM_DELETE = "нажмите «Подтвердить удаление», чтобы удалить |cffffff00%s|r",
PROFILE_CLICK_CONFIRM_RESET  = "нажмите «Подтвердить сброс», чтобы восстановить значения по умолчанию текущего профиля",
PROFILE_NO_NAME_FREE      = "не удалось найти свободное имя профиля",
PROFILE_RELOAD_NOTICE     = "%s — |cffffff00/reload|r, чтобы применить",
PROFILE_SELECTED          = "выбран профиль |cffffff00%s|r",
PROFILE_CREATED           = "создан и выбран профиль |cffffff00%s|r",
PROFILE_WAS_RESET         = "профиль сброшен |cffffff00%s|r",

-- ---------------------------------------------------------------------------
-- Action bars
-- ---------------------------------------------------------------------------
ABC_GROUP                 = "Панели команд",
ABC_GENERAL               = "Общие параметры",
ABC_BAR_N                 = "Панель %d",
ABC_BUTTONS               = "Кнопки",
ABC_BUTTONS_PER_ROW       = "Кнопок в ряду",
ABC_BUTTON_SIZE           = "Размер кнопок",
ABC_BUTTON_SPACING        = "Отступ между кнопками",
ABC_HIDE_SLOT_BACKGROUND  = "Скрыть фон ячеек",
ABC_SHOW_GRYPHONS         = "Показать грифонов",
ABC_CLASSIC_BAR_ACTION_ONLY = "Использовать только панель действий 1",
ABC_CLASSIC_BAR_ACTION_ONLY_HINT = "Выключено: сохраняется полная стандартная панель 1 (грифоны, опыт, микропанель и сумки). Включено: используется панель действий 1 UnrealUI, а отдельный якорь микропанели возвращается после /reload.",
ABC_CLASSIC_BAR_MODE_CHANGED = "Режим панели 1 изменён",
ABC_CLASSIC_BAR_MODE_RELOAD = "Введите /reload, чтобы применить режим панели 1.",
ABC_HINT_BAR1             = "Панель 1 следует за страницей команд и стандартными привязками основной панели.",
ABC_HINT_MULTIBAR         = "Для этих ячеек используются стандартные привязки дополнительных панелей.",
ABC_HINT_PAGE_ONLY        = "Панель только для страниц: в этом клиенте для неё нет команды привязки, поэтому её ячейки работают только мышью.",
ABC_RESERVED_FOR          = "Зарезервировано для: %s",
ABC_RESERVED_ROGUE        = "Незаметность разбойника",
ABC_RESERVED_WARRIOR      = "Стойки воина",
ABC_RESERVED_DRUID        = "Формы друида",
ABC_RESERVED_GENERIC      = "Страницы класса/форм",
ABC_RESERVED_TOOLTIP      = "Зарезервировано для: %s. Панель 1 использует эту страницу автоматически.",
ABC_RESERVED_EXPLANATION  = "Эта страница команд используется панелью 1 автоматически, пока активно состояние «%s». Показ её как отдельной панели вывел бы одни и те же ячейки дважды, поэтому настройки её расположения заблокированы на этом персонаже.",
ABC_RESERVED_SAVED        = "Общие для учётной записи настройки этой страницы сохранены. Они остаются доступны на персонажах, чей класс её не резервирует.",
ABC_SHOW_KEYBIND          = "Показывать привязки клавиш",
ABC_SHOW_MACRO            = "Показывать названия макросов",
ABC_SHOW_COUNT            = "Показывать количество предметов",
ABC_SHOW_COOLDOWN         = "Показывать таймеры восстановления",
ABC_SHOW_GCD              = "Показывать общее время восстановления",
ABC_GENERAL_HINT          = "Доступно независимых панелей: %d. Страницы, используемые формами этого класса, отображаются только на панели 1.",
ABC_BIND_HINT             = "Наведите курсор на ячейку и нажмите клавишу, чтобы назначить её. Escape над ячейкой очищает привязку. Панели 1–5 поддерживают привязки; для панелей 6–10 в этом клиенте нет команды клавиши, поэтому они отображаются, но привязку принять не могут.",
ABC_PET_BAR              = "Панель питомца",
ABC_PET_RESET            = "Вернуть размер клиента",
ABC_PET_HINT             = "Эти настройки размещают собственные кнопки питомца клиента, поэтому здесь нет количества кнопок. Движение любого ползунка передаёт ряд аддону; «Вернуть размер клиента» возвращает его игре. То же самое, что /uui petbar size, spacing и reset.",
ABC_PET_ENABLE_HINT      = "Выключено — панель питомца полностью убирается из интерфейса. Это тот же переключатель, что и её строка в «Дополнительных якорях» в режиме редактирования.",
ABC_PET_UNAVAILABLE      = "Родная панель питомца не активна в этой сессии, поэтому её кнопки нельзя разместить.",

-- ---------------------------------------------------------------------------
-- Quick binding
-- ---------------------------------------------------------------------------
QUICKBIND_TITLE           = "Быстрые привязки",
QUICKBIND_OPENED          = "Быстрые привязки. Наведите курсор на ячейку и нажмите клавишу. |cffffff00Сохранить|r применяет изменения, |cffffff00Отмена|r или Escape отбрасывает их.",
QUICKBIND_NO_ACTIONBARS   = "панели команд недоступны, поэтому быстрым привязкам нечего назначать.",
QUICKBIND_NO_SLOTS        = "ни одна ячейка панели команд не видна, поэтому назначать нечего.",
QUICKBIND_UNAVAILABLE     = "быстрые привязки недоступны в этой сборке.",
QUICKBIND_CLEARED         = "Привязка очищена: панель %s, ячейка %s.",
QUICKBIND_CLEARED_STANCE  = "Привязка очищена: панель стоек, ячейка %s.",
QUICKBIND_NO_COMMAND      = "В этом клиенте нет команды клавиши для панели %s. Привязки поддерживают только панели 1–5.",
QUICKBIND_SAVED_ONE       = "Сохранено %d изменение привязки.",
QUICKBIND_SAVED_FEW       = "Сохранено %d изменения привязок.",
QUICKBIND_SAVED_MANY      = "Сохранено %d изменений привязок.",
QUICKBIND_SAVED_OTHER     = "Сохранено %d изменений привязок.",
QUICKBIND_NOTHING_CHANGED = "Быстрые привязки закрыты. Ничего не изменилось.",
QUICKBIND_REVERTED_ONE    = "Быстрые привязки отменены. %d изменение возвращено.",
QUICKBIND_REVERTED_FEW    = "Быстрые привязки отменены. %d изменения возвращены.",
QUICKBIND_REVERTED_MANY   = "Быстрые привязки отменены. %d изменений возвращено.",
QUICKBIND_REVERTED_OTHER  = "Быстрые привязки отменены. %d изменений возвращено.",
QUICKBIND_CLOSED          = "Быстрые привязки закрыты.",
QUICKBIND_COMBAT          = "Быстрые привязки закрыты: начался бой.",
QUICKBIND_NO_SETBINDING   = "|cffff5555SetBinding недоступен в этом клиенте; быстрые привязки не могут изменять клавиши.|r",
QUICKBIND_SAVE_UNAVAILABLE = "|cffff5555SaveBindings недоступен в этом клиенте:|r клавиши работают сейчас, но не сохранятся после выхода.",
QUICKBIND_SAVE_FAILED     = "|cffff5555Ошибка SaveBindings;|r клавиши работают сейчас, но могут не сохраниться после выхода.",
QUICKBIND_SLOT_COUNT_ONE  = "В этом режиме %d ячейка.",
QUICKBIND_SLOT_COUNT_FEW  = "В этом режиме %d ячейки.",
QUICKBIND_SLOT_COUNT_MANY = "В этом режиме %d ячеек.",
QUICKBIND_SLOT_COUNT_OTHER = "В этом режиме %d ячейки.",

-- ---------------------------------------------------------------------------
-- Unit frames and auras
-- ---------------------------------------------------------------------------
UF_PAGE                   = "Рамки существ",
-- Подразделы группы "Рамки существ" в окне настроек.
UF_TAB_GENERAL            = "Общие настройки",
UF_TAB_PARTY              = "Рамки группы",
UF_TAB_AURAS              = "Эффекты",
UF_TAB_COLORS             = "Цвета",
UF_MENU_WHISPER           = "Шепнуть",

-- Unit frame style (core/unitframestyle.lua). See enUS.lua: the style names
-- stay untranslated, the same as the theme names.
UF_STYLE_HEADER           = "Стиль рамок существ",
UF_STYLE_HINT             = "«Minimal» — стиль рамок, который unrealUI рисует сейчас. «Enhanced» ещё в разработке и пока недоступен.",
UF_STYLE_CHANGED          = "Стиль рамок изменён",
UF_STYLE_RELOAD           = "Введите /reload, чтобы применить стиль рамок «%s».",
UF_COLORS_HEADER          = "Цвета рамок существ",
UF_CUSTOM_BAR_COLORS      = "Использовать свои цвета полос",
UF_HEALTH_BAR_COLOR       = "Цвет полосы здоровья",
UF_CLASS_COLORS           = "Использовать цвета классов для полос здоровья игроков",
UF_POWER_BAR_COLORS       = "Цвета полос ресурса",
UF_POWER_MANA             = "Мана",
UF_POWER_RAGE             = "Ярость",
UF_POWER_FOCUS            = "Фокус",
UF_POWER_ENERGY           = "Энергия",

UF_OFFLINE                = "НЕ В СЕТИ",

UF_EXACT_VITALS_HEADER    = "Здоровье существ",
UF_EXACT_VITALS           = "Показывать точное здоровье и ману",
UF_EXACT_VITALS_HINT      = "Клиент сообщает здоровье существа только в процентах. Когда опция включена, рамка цели показывает реальные значения из встроенной таблицы существ и уточняет текущее по нанесённому урону. Выключите, чтобы видеть проценты.",

UF_PARTY_HEADER           = "Рамки группы",
UF_PARTY_PLAYER           = "Показывать себя в рамках группы",
UF_PARTY_PETS             = "Показывать питомцев членов группы",
UF_POWER_TICK_HEADER      = "Тик ресурса игрока",
UF_MANA_TICK              = "Показывать тик маны",
UF_ENERGY_TICK            = "Показывать тик энергии",
UF_COMBO_POINTS_HEADER    = "Серия приемов",
UF_COMBO_POINTS_PLAYER_FRAME = "Рамка игрока",
UF_COMBO_POINTS_TARGET_FRAME = "Рамка цели",
UF_MOVER_INCOMING_HEALS   = "Входящее лечение",
UF_MOVER_AURAS_ENABLED    = "Показывать эффекты",
UF_MOVER_AURAS_TIMERS     = "Таймеры эффектов",
UF_MOVER_AURAS_BELOW      = "Эффекты под рамками",
UF_MOVER_EXACT_VITALS     = "Точное здоровье",
UF_MOVER_PORTRAIT_3D      = "3D-портрет",

-- Живая 3D-модель в кольце портрета Modern WoW (modules/unitframes.lua
-- model3d); круглый каменный фон остаётся под ней.
UF_PORTRAIT_HEADER        = "Портреты",
UF_PORTRAIT_3D_PLAYER     = "3D-портрет игрока",
UF_PORTRAIT_3D_TARGET     = "3D-портрет цели",
UF_PORTRAIT_3D_TARGET_TARGET = "3D-портрет цели цели",
UF_PORTRAIT_3D_PARTY      = "3D-портреты группы",
UF_PORTRAIT_3D_HINT       = "Для тем Modern и Modern WoW. Для каждой группы рамок 3D-модель включается отдельно: в квадрате рядом с рамкой в теме Modern, поверх каменного фона в теме Modern WoW. 2D-портрет скрыт, кроме слишком удалённых участников группы.",

-- Индикаторы периодического лечения на рамках группы (modules/hots.lua).
HOTS_HEADER               = "Индикаторы периодического лечения",
HOTS_ENABLED              = "Показывать периодическое лечение",
HOTS_CORNER               = "Угол",
HOTS_SIZE                 = "Размер значков",
HOTS_SPACING              = "Отступ между значками",
HOTS_CORNER_TOPLEFT       = "Сверху слева",
HOTS_CORNER_TOPRIGHT      = "Сверху справа",
HOTS_CORNER_BOTTOMLEFT    = "Снизу слева",
HOTS_CORNER_BOTTOMRIGHT   = "Снизу справа",
HEALPREDICT_HEADER        = "Входящее лечение",
HEALPREDICT_ENABLED       = "Показывать входящее лечение на полосах здоровья",
HEALPREDICT_HINT          = "Закрашивает ту часть полосы здоровья, которую заполнит ваше лечение, пока оно ещё читается, чтобы избыточное лечение было видно заранее. Клиент не сообщает о входящем лечении, поэтому показываются только ваши собственные заклинания, а величина лечения становится известна лишь после того, как оно один раз сработало. Лечение других лекарей никогда не отображается.",

AURAS_HEADER              = "Эффекты на рамках существ",
AURAS_ON_PLAYER_FRAME     = "Показывать эффекты на рамке игрока",
AURAS_ON_TARGET_FRAME     = "Показывать эффекты на рамке цели",
AURAS_NEAR_MINIMAP        = "Показывать эффекты у миникарты",
AURAS_PLAYER_DEBUFFS      = "Дебаффы на рамке игрока",
AURAS_PLAYER_BUFFS        = "Баффы на рамке игрока",
AURAS_TARGET_DEBUFFS      = "Дебаффы на рамке цели",
AURAS_TARGET_BUFFS        = "Баффы на рамке цели",
AURAS_PARTY_DEBUFFS       = "Дебаффы на рамках группы",
AURAS_PARTY_BUFFS         = "Баффы на рамках группы",
AURAS_SHOW_TIMERS         = "Таймеры на значках эффектов",
AURAS_BELOW_FRAME         = "Эффекты игрока и цели под рамками",
AURAS_DISPEL_HEADER       = "Показывать дебаффы по типу рассеивания",
AURAS_MAGIC               = "Магия",
AURAS_CURSE               = "Проклятие",
AURAS_POISON              = "Яд",
AURAS_DISEASE             = "Болезнь",
AURAS_OTHER               = "Физический / прочее",
AURAS_HINT                = "Точны только таймеры ваших собственных эффектов. Для других существ клиент не сообщает длительность, поэтому она оценивается по таблице заклинаний: уже действующий эффект выглядит только что наложенным, а отсутствующий в таблице — без таймера. Введите /uui aura, чтобы увидеть, какой из них какой.",

-- ---------------------------------------------------------------------------
-- Bags and bank
-- ---------------------------------------------------------------------------
SETTINGS_BAGS_ENABLE      = "Включить",
SETTINGS_BAGS_ENABLE_HINT = "Заменяет окна сумок клиента общей сумкой UnrealUI. Если снять флажок, сумка UnrealUI полностью отключается и возвращаются стандартные сумки клиента, чтобы другой аддон для сумок мог работать без конфликтов. Любое изменение требует перезагрузки.",
SETTINGS_BAGS_CHANGED     = "Интерфейс сумок изменён",
SETTINGS_BAGS_RELOAD_ON   = "Введите /reload, чтобы использовать сумку UnrealUI.",
SETTINGS_BAGS_RELOAD_OFF  = "Введите /reload, чтобы вернуть стандартные сумки клиента.",
SETTINGS_BAGS_CATEGORIES  = "Сумка по категориям",
SETTINGS_BAGS_CATEGORIES_HINT = "Группирует содержимое сумки и банка в подписанные блоки категорий вместо единой сетки. Использует данные предметов, включая отдельные группы ткани, трав, мяса и руды. Применяется сразу, перезагрузка не требуется.",
-- HUD-панель сумок (modules/bagbar.lua): замена органов управления сумками,
-- пока модуль сумок отключён.
SETTINGS_BAGBAR_ENABLE      = "Показывать панель сумок на экране",
SETTINGS_BAGBAR_ENABLE_HINT = "Помещает кнопку рюкзака, четыре ячейки надетых сумок и связку ключей в угол экрана, пока сумка UnrealUI выше отключена. Без этого на экране не останется ни одного органа управления сумками: панели действий UnrealUI заменяют родную панель, на которой клиент держит кнопки сумок. При включённой сумке UnrealUI ничего не меняет; в обоих случаях нужна перезагрузка.",
SETTINGS_BAGBAR_RELOAD      = "Введите /reload, чтобы применить изменение панели сумок.",
BAGBAR_BACKPACK             = "Сумки",
BAGBAR_FREE_SLOTS           = "Свободно %d из %d",
BAGBAR_OPEN_ALL_HINT        = "Правый клик открывает все сумки.",
BAGBAR_FREE_FORMAT          = "%d/%d",

ITEM_CATEGORY_FAVORITE    = "Избранное",
ITEM_CATEGORY_GEAR        = "Моё снаряжение",
ITEM_CATEGORY_QUEST       = "Предметы заданий",
ITEM_CATEGORY_CONSUMABLE  = "Расходники",
ITEM_CATEGORY_FOOD        = "Еда и напитки",
ITEM_CATEGORY_POTION      = "Зелья",
ITEM_CATEGORY_BUFF        = "Усиления",
ITEM_CATEGORY_BANDAGE     = "Бинты",
ITEM_CATEGORY_EXPLOSIVE   = "Взрывчатка",
ITEM_CATEGORY_REAGENT     = "Реагенты класса",
ITEM_CATEGORY_LEATHER     = "Кожа",
ITEM_CATEGORY_CLOTH       = "Ткань",
ITEM_CATEGORY_HERB        = "Травы",
ITEM_CATEGORY_ORE         = "Руда и камень",
ITEM_CATEGORY_GEM         = "Самоцветы",
ITEM_CATEGORY_ENCHANTING  = "Материалы для наложения чар",
ITEM_CATEGORY_ENGINEERING = "Детали для инженерного дела",
ITEM_CATEGORY_MEAT        = "Мясо",
ITEM_CATEGORY_TRADEGOODS  = "Хозяйственные товары",
ITEM_CATEGORY_RECIPE      = "Рецепты",
ITEM_CATEGORY_AMMO        = "Боеприпасы",
ITEM_CATEGORY_CONTAINER   = "Контейнеры",
ITEM_CATEGORY_KEY         = "Ключи",
ITEM_CATEGORY_JUNK        = "Хлам",
ITEM_CATEGORY_MISC        = "Разное",
ITEM_CATEGORY_UNKNOWN     = "Без категории",

BAGS_TITLE                = "Сумки",
LOOT_TITLE                = "Предметы",
LOOT_QUALITY_0            = "Низкое",
LOOT_QUALITY_1            = "Обычное",
LOOT_QUALITY_2            = "Необычное",
LOOT_QUALITY_3            = "Редкое",
LOOT_QUALITY_4            = "Эпическое",
LOOT_QUALITY_5            = "Легендарное",
LOOT_QUALITY_6            = "Артефакт",
BAGS_SLOT_COUNT           = "%d/%d",
BAGS_CATEGORY_EMPTY       = "Пустые",
BAGS_SORT                 = "Сортировка сумок",
BAGS_SORT_HINT            = "Переставляет содержимое сумок по категориям. Специальные сумки (травы, души, чары, инженерное дело, колчаны) не затрагиваются.",
BAGS_SORT_BUSY            = "Сортировка уже идёт.",
BAGS_SORT_CURSOR          = "Сначала положите предмет с курсора.",
BAGS_SORT_DONE            = "Сумки отсортированы.",
BAGS_SORT_NOTHING         = "Сумки уже отсортированы.",
BAGS_SORT_FAILED          = "Сортировка остановлена: предмет не переместился. Ничего не потеряно.",
BAGS_SORT_NOSTAGE         = "Для сортировки нужна одна свободная ячейка в сумке. Освободите ячейку и повторите.",
BAGS_INVENTORY_FULL       = "Нет подходящей свободной ячейки в сумках.",
ITEMS_BUSY                = "Дождитесь окончания текущей сортировки или объединения.",
BAGS_STACK                = "Объединить стопки",
BAGS_STACK_HINT           = "Объединяет неполные стопки одного предмета в сумках. Предметы в банке не затрагиваются.",
BAGS_BANK_VIEW            = "Сохранённый банк",
BAGS_BANK_VIEW_HINT       = "Показывает банк таким, каким он был при последнем посещении банкира. Только просмотр.",
BAGS_STACK_CURSOR         = "Сначала положите предмет с курсора.",
BAGS_STACK_DONE           = "Стопки в сумках объединены.",
BAGS_STACK_NOTHING        = "В сумках нечего объединять.",
BAGS_STACK_FAILED         = "Объединение остановлено: предмет не переместился. Ничего не потеряно.",
BAGS_SEARCH               = "Поиск",
BAGS_TOGGLE_KEYRING       = "Показать/скрыть связку ключей",
BAGS_KEYRING_HINT         = "Показать связку ключей.",
BAGS_TOGGLE_BAGS          = "Показать/скрыть сумки",
BAGS_BAG_SLOTS_HINT       = "Показать ячейки надетых сумок.",
BAGS_VENDOR_GRAYS         = "Продать / удалить серые предметы",
BAGS_GREYS_HINT           = "Продаёт серые предметы у торговца; в остальных случаях предлагает удалить их.",
BAGS_PICK_LOCK            = "Взлом замка",
BAGS_PICK_LOCK_HINT       = "Активируйте «Взлом замка», затем нажмите на запертый ящик в сумках.",
BAGS_PICK_LOCK_ACTION_HINT = "Поместите «Взлом замка» на панель команд, чтобы использовать кнопку у сумок.",
BAGS_DELETE_CONFIRM_ONE   = "Удалить %d серый предмет?",
BAGS_DELETE_CONFIRM_FEW   = "Удалить %d серых предмета?",
BAGS_DELETE_CONFIRM_MANY  = "Удалить %d серых предметов?",
BAGS_DELETE_CONFIRM_OTHER = "Удалить %d серых предметов?",
BAGS_SOLD_GREYS           = "Серые предметы проданы за %s.",
BAGS_DELETED_GREYS_ONE    = "Удалён %d серый предмет.",
BAGS_DELETED_GREYS_FEW    = "Удалено %d серых предмета.",
BAGS_DELETED_GREYS_MANY   = "Удалено %d серых предметов.",
BAGS_DELETED_GREYS_OTHER  = "Удалено %d серых предмета.",
BAGS_NO_GREYS             = "Серых предметов не найдено.",
BAGS_FAVORITE_HINT_ADD    = "Alt + ЛКМ: отметить как избранное",
BAGS_FAVORITE_HINT_REMOVE = "Alt + ЛКМ: убрать из избранного",
BAGS_FAVORITE_SELL_CONFIRM = "Продать этот избранный предмет?",
BAGS_FAVORITE_SELL_ACCEPT = "Продать",
BAGS_FAVORITE_BATCH_SELL_ONE   = "%d серый предмет отмечен как избранный.",
BAGS_FAVORITE_BATCH_SELL_FEW   = "%d серых предмета отмечены как избранные.",
BAGS_FAVORITE_BATCH_SELL_MANY  = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_SELL_OTHER = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_ONE   = "%d серый предмет отмечен как избранный.",
BAGS_FAVORITE_BATCH_DELETE_FEW   = "%d серых предмета отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_MANY  = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DELETE_OTHER = "%d серых предметов отмечены как избранные.",
BAGS_FAVORITE_BATCH_DETAIL = "Включить их или пропустить и оставить в сумках?",
BAGS_FAVORITE_BATCH_INCLUDE = "Включить",
BAGS_FAVORITE_BATCH_SKIP  = "Пропустить",
BAGS_FAVORITE_BATCH_NONE_LEFT = "Найдены только избранные серые предметы; ничего не сделано.",
BANK_FULL                 = "Нет подходящей свободной ячейки в банке.",
BANK_TITLE                = "Банк",
BANK_SORT                 = "Сортировка банка",
BANK_SORT_HINT            = "Переставляет содержимое банка и банковских сумок по категориям. Специальные сумки не затрагиваются.",
BANK_SORT_BUSY            = "Сортировка уже идёт.",
BANK_SORT_CURSOR          = "Сначала положите предмет с курсора.",
BANK_SORT_DONE            = "Банк отсортирован.",
BANK_SORT_NOTHING         = "Банк уже отсортирован.",
BANK_SORT_FAILED          = "Сортировка банка остановлена: предмет не переместился.",
BANK_SORT_NOSTAGE         = "Для сортировки банка нужна одна свободная ячейка в банковской сумке. Освободите ячейку и повторите.",
BANK_STACK                = "Объединить стопки в банке",
BANK_STACK_HINT           = "Объединяет неполные стопки одного предмета в банке и банковских сумках. Предметы остаются в банке. Для стопок в основном банке нужна одна свободная ячейка в банковской сумке.",
BANK_STACK_CURSOR         = "Сначала положите предмет с курсора.",
BANK_STACK_DONE           = "Стопки в банке объединены.",
BANK_STACK_NOTHING        = "В банке нечего объединять.",
BANK_STACK_FAILED         = "Объединение в банке остановлено: предмет не переместился. Ничего не потеряно.",
BANK_STACK_NOSTAGE        = "Для объединения некоторых стопок основного банка нужна одна свободная ячейка в банковской сумке. Освободите ячейку и повторите.",
BANK_BAG_LABEL            = "Банковская сумка",
BANK_BUY_SLOT             = "Купить ещё одну ячейку для банковской сумки?",
BANK_PURCHASE             = "Купить",
BANK_PURCHASE_TITLE       = "Купить банковскую сумку",
BANK_TRANSFER_PICKUP      = "Перенос через банк остановлен в вашем инвентаре; снова возьмите предмет, чтобы продолжить.",
BANK_TRANSFER_EMPTY_SLOT  = "Для переноса предметов между банковскими сумками нужна промежуточная ячейка в сумках персонажа; подходящей не нашлось.",
BANK_SLOT_TEMPLATE_FALLBACK = "%s недоступен в этом клиенте; для ячеек банка используется шаблон ячеек сумки.",
BANK_SLOTS_LOADING        = "Клиент ещё не сообщил ячейки банка; повторяем попытку, пока окно открыто.",
BANK_VIEW_TITLE           = "Сохранённый банк",
BANK_VIEW_READONLY        = "Только просмотр",
BANK_VIEW_EMPTY           = "Банк ещё не записан. Посетите банкира в первый раз, чтобы его записать.",

-- ---------------------------------------------------------------------------
-- Item comparison (modules/tooltip.lua)
-- ---------------------------------------------------------------------------
TOOLTIP_COMPARE_SUMMARY   = "Если заменить этот предмет:",

-- ---------------------------------------------------------------------------
-- Overlays and native screens
-- ---------------------------------------------------------------------------
STATUS_FPS                = "Кадр/с:",
STATUS_LATENCY            = "Мс:",
STATUS_DURABILITY         = "Прочность:",
STATUS_ONLINE             = "игроков в сети",

WORLDMAP_CURSOR           = "Курсор: --, --",
WORLDMAP_CURSOR_OFF_MAP   = "Курсор: вне карты",
SPELLBOOK_HIGHEST_RANK    = "Высший ранг",
SPELLBOOK_HIGHEST_RANK_TOOLTIP = "Показывает только последний ранг каждого заклинания.",
SPELLBOOK_BAR_HINT        = "Нет на панелях",
SPELLBOOK_BAR_HINT_TOOLTIP = "Подсвечивает заклинания вне панелей.",
SPELLBOOK_TAB             = "Книга заклинаний",
SPELLBOOK_PROFESSIONS     = "Профессии",
SPELLBOOK_PROF_FIRST      = "Первая профессия",
SPELLBOOK_PROF_SECOND     = "Вторая профессия",
SPELLBOOK_PROF_MISSING    = "Посетите учителя, чтобы изучить профессию.",
SPELLBOOK_PROF_NOT_LEARNED = "Не изучено",
SPELLBOOK_PROF_FISHING    = "Рыбная ловля",
SPELLBOOK_PROF_COOKING    = "Кулинария",
SPELLBOOK_PROF_FIRST_AID  = "Первая помощь",
SPELLBOOK_PROF_RANK_1     = "Ученик",
SPELLBOOK_PROF_RANK_2     = "Подмастерье",
SPELLBOOK_PROF_RANK_3     = "Умелец",
SPELLBOOK_PROF_RANK_4     = "Мастеровой",
SPELLBOOK_PROF_UNLEARN    = "Забыть",
SPELLBOOK_PROF_UNLEARN_CONFIRM = "Забыть профессию %s?",
SPELLBOOK_PROF_UNLEARN_DETAIL = "Весь навык и известные рецепты будут потеряны.",
TALENTS_TITLE             = "Основные таланты - активны",
TALENTS_UNSPENT_POINTS    = "Свободные очки талантов: |cffffd100%d|r",
TALENTS_NEXT_LEVEL        = "Следующий талант доступен на |cffffd100уровне %d|r",
TALENTS_ROLE_DAMAGER     = "Урон",
TALENTS_ROLE_TANK        = "Танк",
TALENTS_ROLE_HEALER      = "Лечение",
TA_TITLE                 = "Советник талантов",
TA_CONTEXT_LEVELING      = "Прокачка",
TA_CONTEXT_PVP           = "PvP",
TA_CONTEXT_RAID          = "Рейд",
TA_ROLE_DPS              = "Урон",
TA_ROLE_TANK             = "Танк",
TA_ROLE_HEALER           = "Лекарь",
TA_ROLE_HYBRID           = "Гибрид",
TA_LEGEND                = "|cffffd119Золото|r: сейчас / |cffffffffБелый|r: позже / |cffff3838Красный|r: вне сборки",
TA_STATUS_NEXT           = "Следующий: %s (%d/%d)",
TA_STATUS_COMPLETE       = "Эта сборка завершена.",
TA_STATUS_EARN_POINT     = "Уровень %d: осталось %d очков. Получите очко таланта.",
TA_STATUS_RESPEC         = "Сейчас нельзя изучить рекомендованный талант.",
TA_STATUS_OUTSIDE        = "%d очк. вне этой сборки; может потребоваться сброс.",
TA_STATUS_UNAVAILABLE    = "Сборка не соответствует данным талантов клиента.",
TA_BUILD_ARMS            = "Оружие с двуручным оружием",
TA_BUILD_FURY            = "Неистовство с двумя оружиями",
TA_BUILD_PROTECTION      = "Глубокая защита",
TA_BUILD_FERAL           = "Сила зверя",
TA_BUILD_HOTW_NS         = "Сердце дикой природы / Природная стремительность",
TA_BUILD_BALANCE_NS      = "Баланс / Природная стремительность",
TA_BUILD_MOONGLOW        = "Лунное сияние",
TA_BUILD_RESTORATION     = "Глубокое восстановление",
TA_BUILD_BEAST_MASTERY   = "Повелитель зверей",
TA_BUILD_MARKSMAN        = "Стрельба",
TA_BUILD_MARKSMAN_SURVIVAL = "Стрельба / Выживание",
TA_BUILD_FROST           = "Лед",
TA_BUILD_POM_FIRE        = "Присутствие разума / Огонь",
TA_BUILD_AP_FROST        = "Мощь тайной магии / Лед",
TA_BUILD_WINTERS_CHILL   = "Зимняя стужа",
TA_BUILD_FIRE            = "Огонь / Возгорание",
TA_BUILD_RETRIBUTION     = "Воздаяние",
TA_BUILD_HOLY_PVP        = "Свет PvP",
TA_BUILD_HOLY            = "Свет",
TA_BUILD_HOLY_RETRIBUTION = "Свет / Воздаяние",
TA_BUILD_SHADOW_LEVELING = "Тьма для прокачки",
TA_BUILD_SHADOW          = "Тьма",
TA_BUILD_DISCIPLINE      = "Глубокая дисциплина",
TA_BUILD_COMBAT_SWORDS   = "Бой на мечах",
TA_BUILD_HEMORRHAGE      = "Хладнокровие / Подготовка / Кровоизлияние",
TA_BUILD_COMBAT_DAGGERS  = "Бой с кинжалами",
TA_BUILD_ENHANCEMENT     = "Совершенствование",
TA_BUILD_ELEMENTAL_NS    = "Стихии / Природная стремительность",
TA_BUILD_ENH_RESTO       = "Совершенствование / Исцеление",
TA_BUILD_AFFLICTION      = "Колдовство / Демонология",
TA_BUILD_SOUL_LINK       = "Связка души",
TA_BUILD_DS_RUIN         = "Демоническое жертвоприношение / Разорение",
TA_BUILD_SM_RUIN         = "Власть над Тьмой / Разорение",
PROFESSIONS_REQUIRES      = "Требуется:",
PROFESSIONS_REAGENTS      = "Реагенты:",
PROFESSIONS_FILTER_GRAY   = "Серый",
PROFESSIONS_FILTER_GREEN  = "Зеленый",
PROFESSIONS_FILTER_YELLOW = "Желтый",
PROFESSIONS_FILTER_ORANGE = "Оранжевый",
PROFESSIONS_FILTER_ALL_REAGENTS = "Все реагенты в наличии",
PROFESSIONS_FILTER_DIFFICULTY = "Сложность",
PROFESSIONS_TRACK_RECIPE  = "Отслеживать рецепт",
CRAFT_TRACKER_FULL        = "Отслеживание ремесла: можно отслеживать не более %d рецептов.",
PROFESSIONS_CREATE        = "Создать",
PROFESSIONS_CREATE_ALL    = "Создать все",
PROFESSIONS_COOLDOWN      = "До восстановления: %s",
PROFESSIONS_TRAINING_POINTS = "Очки обучения: %d",
PROFESSIONS_NO_RECIPES    = "Нет рецептов для показа.",
GAMEMENU_OPTIONS          = "Настройки",
GAMEMENU_RETURN           = "Вернуться в игру",
GAMEMENU_TITLE            = "Игровое меню",
GAMEMENU_EDIT_MODE        = "Режим редактирования",
GAMEMENU_SUPPORT          = "Поддержка",
GAMEMENU_SUPPORT_UNAVAILABLE = "окно поддержки недоступно в этой сборке.",

-- ---------------------------------------------------------------------------
-- Experience bar settings and tooltip (modules/xpbar.lua)
-- ---------------------------------------------------------------------------
XPBAR_WIDTH               = "Ширина полосы",
XPBAR_HEIGHT              = "Высота полосы",
XPBAR_SHOW_TEXT           = "Показывать текст опыта",
XPBAR_TEXT_FORMAT         = "%d / %d (%d%%)",
XPTIP_TITLE               = "Опыт",
XPTIP_XP                  = "Опыт",
XPTIP_REMAINING           = "Осталось",
XPTIP_STATUS              = "Состояние",
XPTIP_RESTING             = "Отдых",
XPTIP_RESTED              = "Отдохнувший",
XPTIP_SESSION             = "За сессию",
XPTIP_PER_HOUR            = "В среднем за час",
XPTIP_TIME_LEFT           = "Осталось времени",
-- Single-letter duration suffixes for the Time Remaining readout, e.g. "2h 15m".
XPTIP_UNIT_DAY            = "д",
XPTIP_UNIT_HOUR           = "ч",
XPTIP_UNIT_MINUTE         = "м",
XPTIP_UNIT_SECOND         = "с",

-- ---------------------------------------------------------------------------
-- Slash commands
-- ---------------------------------------------------------------------------
CMD_HEADER                = "v%s, команды:",
CMD_SETTINGS              = "  |cffffff00/uui|r — открыть настройки",
CMD_GAME                  = "  |cffffff00/uui game|r — открыть настройки игры",
CMD_UNLOCK                = "  |cffffff00/uui unlock|r — разблокировать рамки для перемещения",
CMD_LOCK                  = "  |cffffff00/uui lock|r — заблокировать рамки",
CMD_RESET                 = "  |cffffff00/uui reset|r — сбросить все позиции рамок",
CMD_BIND                  = "  |cffffff00/uui bind|r — открыть быстрые привязки",
CMD_CHECK                 = "  |cffffff00/uui check|r — самопроверка среды выполнения",
CMD_THEME                 = "  |cffffff00/uui theme <style>|r — выбрать оформление (нужен /reload)",
CMD_LANGUAGE              = "  |cffffff00/uui lang <en/cn/ru/fr>|r — выбрать язык (нужен /reload)",
CMD_PROFILE               = "  |cffffff00/uui profile create <имя>|r — создать и выбрать общий профиль",
CMD_DEBUG                 = "  |cffffff00/uui debug|r — включить/выключить отладочный вывод",
CMD_DIAGNOSTICS           = "  |cffffff00/uui help diag|r — список диагностических дампов",
CMD_UNKNOWN               = "неизвестная команда: %s",
CMD_SETTINGS_UNAVAILABLE  = "интерфейс настроек пока недоступен в этой сборке.",
CMD_LANGUAGE_CURRENT      = "язык: |cffffff00%s|r",
CMD_LANGUAGE_AVAILABLE    = "доступные языки:",
CMD_PROFILE_CURRENT       = "текущий профиль: |cffffff00%s|r",
CMD_PROFILE_CREATE_USAGE  = "  |cffffff00/uui profile create <имя>|r",
CMD_PROFILE_UNAVAILABLE   = "управление профилями недоступно в этой сборке",
CMD_PROFILE_EXISTS        = "профиль с именем |cffffff00%s|r уже существует",
CMD_PROFILE_NAME_RULES    = "имя профиля может содержать буквы, цифры, пробелы, дефисы и подчёркивания (не более 64 символов)",
CMD_PROFILE_CREATED       = "создан и выбран профиль |cffffff00%s|r — |cffffff00/reload|r, чтобы применить",

-- Quest Log count boxes (modern-wow theme).
QUESTLOG_COUNT            = "Задания: |cffffffff%d/%d|r",
QUESTLOG_COMPLETED        = "Выполнено: |cffffffff%d/%d|r",
MERCHANT_PAGE_COUNT       = "Страница %d из %d",

})

-- Russian counted-noun forms. The rule is the standard CLDR one and depends on
-- the last two digits, so 11-14 take the "many" form even though they end in
-- 1-4: 21 позиция, but 11 позиций.
UnrealUI.RegisterLocalePlural("ruRU", function(n)
  if n ~= math.floor(n) then return "OTHER" end

  local mod10 = math.mod(n, 10)
  local mod100 = math.mod(n, 100)

  if mod10 == 1 and mod100 ~= 11 then return "ONE" end
  if mod10 >= 2 and mod10 <= 4 and (mod100 < 12 or mod100 > 14) then
    return "FEW"
  end
  return "MANY"
end)
