...
local function PlaceModernWowPanes()
  ...
  if listScroll then
    pcall(function()
      ...
      listScroll:SetPoint("TOPLEFT", frame, "TOPLEFT", MW_LIST_LEFT, MW_LIST_TOP)
      listScroll:SetHeight(322 + listGrow)
    end)
  end
...
-- scroll child under modern-wow. When UnrealQuest is absent, replace it with
...
function modernWowMoney.RelativeName(relative)
...
function modernWowMoney.RestoreMovedItem()
  local item = modernWowMoney.movedItem
  modernWowMoney.movedItem = nil
  ...
  if not ok or modernWowMoney.RelativeName(relative) ~= modernWowMoney.name then
    return false
  end
  ...
  return pcall(function()
    ...
    item:SetPoint(point or "TOPLEFT", label, relativePoint or "BOTTOMLEFT",
                  type(x) == "number" and x or 0, -modernWowMoney.itemGap)
  end)
end

function modernWowMoney.Create(dock)
  if modernWowMoney.row ~= nil then return modernWowMoney.row or nil end
  local ok, row = pcall(CreateFrame, "Frame", modernWowMoney.name, dock)
  if not ok or not row then
    modernWowMoney.row = false
    return nil
  end
  ...
  modernWowMoney.row = row
  modernWowMoney.readout = readout
  return row
end

function modernWowMoney.RewardCounts()
...
function modernWowMoney.PlaceRewardItem(row)
  local choices, rewards = modernWowMoney.RewardCounts()
  if rewards <= 0 then return modernWowMoney.RestoreMovedItem() end

  local item = G("QuestLogItem" .. (choices + 1))
  if not item or type(item.GetPoint) ~= "function" then return false end
  if modernWowMoney.movedItem and modernWowMoney.movedItem ~= item then
    modernWowMoney.RestoreMovedItem()
  end

  local ok, point, relative, relativePoint, x = pcall(item.GetPoint, item, 1)
  if not ok then return false end
  local relativeName = modernWowMoney.RelativeName(relative)
  if relativeName == modernWowMoney.name then
    modernWowMoney.movedItem = item
    return false
  end
  ...
  local placed = pcall(function()
    ...
    item:SetPoint(point or "TOPLEFT", row, relativePoint or "BOTTOMLEFT",
                  ...
                  -modernWowMoney.itemGap)
  end)
  if placed then modernWowMoney.movedItem = item end
  return placed and true or false
end

function modernWowMoney.ReadAmount()
...
function modernWowMoney.GeometryChanged(row, dock)
  ...
  local changed = modernWowMoney.relativeTop ~= relativeTop
                  or modernWowMoney.relativeBottom ~= relativeBottom
  modernWowMoney.relativeTop = relativeTop
  modernWowMoney.relativeBottom = relativeBottom
  return changed
end
...
-- replacement marker.
function modernWowMoney.Release()
  modernWowMoney.RestoreMovedItem()
  if modernWowMoney.row and IsShown(modernWowMoney.row) then
    pcall(modernWowMoney.row.Hide, modernWowMoney.row)
  end
  modernWowMoney.amount = nil
  modernWowMoney.anchored = false
  modernWowMoney.relativeTop = nil
  modernWowMoney.relativeBottom = nil
  ...
  if native and native.uuiModernWowMoneyReplaced then
    ...
    if not native.unrealQuestMoneyReplaced then
      ...
      local amount = modernWowMoney.ReadAmount()
      if amount and amount > 0 then pcall(native.Show, native) end
    end
  end
end

function modernWowMoney.Refresh()
  if not ModernWow() then return false end
  if G("UnrealQuest") then
    modernWowMoney.Release()
    return false
  end
  ...
  local changed = false
  local row = modernWowMoney.row or nil
  if wanted and modernWowMoney.row == nil then
    row = modernWowMoney.Create(dock)
  end
  if not wanted or not row then
    changed = modernWowMoney.RestoreMovedItem()
    ...
    modernWowMoney.relativeTop = nil
    modernWowMoney.relativeBottom = nil
    if changed and type(detail.UpdateScrollChildRect) == "function" then
      pcall(detail.UpdateScrollChildRect, detail)
    end
    return false
  end
  ...
  if modernWowMoney.amount ~= amount then
    modernWowMoney.readout:SetAmount(amount)
    row:SetWidth(modernWowMoney.readout.contentWidth or 1)
    modernWowMoney.amount = amount
    changed = true
  end
  if not modernWowMoney.anchored then
    modernWowMoney.anchored = pcall(function()
      row:ClearAllPoints()
      row:SetPoint("TOPLEFT", label, "BOTTOMLEFT", 0, -2)
    end)
    changed = modernWowMoney.anchored or changed
  end
  if not IsShown(row) then pcall(row.Show, row) end
  changed = modernWowMoney.GeometryChanged(row, dock) or changed
  changed = modernWowMoney.PlaceRewardItem(row) or changed
...
function fitRows.Apply()
  if fitRows.busy or not ModernWow() or not listScroll or not IsShown(frame) then
    return
  end
  ...
  local okPane, paneBottom = pcall(listScroll.GetBottom, listScroll)
...
function questCount.Build(x, centerY)
  ...
  local quests = questCount.CreateBox(frame)
  local completed = quests and questCount.CreateBox(frame)
  ...
  pcall(function()
    if centerY then
      quests:SetPoint("LEFT", frame, "TOPLEFT", x, centerY)
    else
      quests:SetPoint("TOPLEFT", frame, "TOPLEFT", x, spec.y)
    end
...
local function DressModernWow(panels, buttons)
  ...
  if collapseAllButton and ringLeft then
    pcall(function()
      ...
      collapseAllButton:SetPoint("BOTTOMLEFT", frame, "TOPLEFT",
                                 ringLeft + ringWidth + MW_RING_GAP,
                                 MW_LIST_TOP + MW_COLLAPSE_ALL_GAP
                                   - MW_COLLAPSE_ALL_FACE_DROP)
      collapseAllButton:SetHitRectInsets(0, 0, -MW_COLLAPSE_ALL_FACE_DROP,
                                         MW_COLLAPSE_ALL_FACE_DROP)
    end)
  ...
  if ringLeft and collapseAllButton then
    local spec = M.modernWow.questLog.countBox
    local okW, w = pcall(collapseAllButton.GetWidth, collapseAllButton)
    local okH, h = pcall(collapseAllButton.GetHeight, collapseAllButton)
...
local function UpdateRows()
  ...
  -- No-op until the modern-wow boxes exist.
  questCount.Update()
end
...
local function BuildFrame()
  ...
  if ModernWow() then
    ...
    U.RegisterUpdate("questlog.modernwow-money", 0.3, function()
      ...
      if IsShown(frame) then modernWowMoney.Refresh() end
    end)
  end
